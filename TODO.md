...
## Android 1.72 をクローズドテストの審査に送信（2026-09-15 07:58）

- Closed beta トラック・リリース 5「172 (1.72)」。1.71 と入れ替え、段階的公開 100%、管理対象の公開はオフ（承認されたらそのまま配信）
- AAB は `backup/mitodiet-172-release.aab`。送信前に中の `index.html` が HEAD（`3dcc504`、その後の `b98d67b` は TODO.md だけ）と一致することを md5 で確認
- 警告は「難読化解除ファイルなし」の1件のみ。サポート対象外になった端末 0
- リリースノート ja-JP（8項目、`docs/handoff_2026-09-15_v172.md` の案どおり。入力中に猫がキーボードを踏んで3か所欠けたのを直し、期待文と完全一致を確認）
- 送信時点で「一般的な問題のクイック チェック」残り約14分。チェック完了後に自動で審査へ回る
- **未確認: 承認・配信の時刻**（1.71 は送信から約15分だった）

## 1.72 の最終 QA で見つかった小さな問題（1.73 以降）
- 通知バナー（例「最後のバックアップから53日…」）を表示中に言語を切り替えると、その通知だけ切り替え前の言語のまま残る。最初からその言語で起動すれば正しく出る。表示中の通知を `setLang` のときに出し直すか消す
- zh / ms 端末で言語を選んだことのない既存ユーザーは、1.72 に更新すると英語から中国語／マレー語に変わる（appLang の判定を広げたため。意図どおり）
- iOS の 1.72 相当（アプリ名 InfoPlist.strings・@capacitor/app）は未ビルド。次に iOS を出すときに実機で確認
...
- **実機で確認済み（2026-09-14）**: Lenovo Idea Tab Pro に 1.71 を Play から入れ、平井さんが「いい感じになった」と確認。この端末では縦固定は問題にならなかった。以下は当時の懸念の記録: Android 側は `screenOrientation="portrait"` のまま。写真では 1.70 が横長の全画面で出ていた（左右の帯なし）ので、この端末では縦固定が効いていないと推測（Android 16 の大画面ルールの可能性。端末の Android バージョンは未確認）。横持ちで2列、縦持ちで720px幅の1列になるはず。**帯つきの縦長窓で出たら、縦固定の解除が別に必要。**
